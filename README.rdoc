...
* Automatic geo location lookup for node based on given postal address
...
* Map displaying nodes
  ...
  * Additional layer to display data from OLSR nameservice plugin
  * Additional layer to display data from global freifunkmap on layereight.de
...
* Auto-generation/proposal of IP addresses 
* Pagination (will_paginate)
* Tags (acts_as_taggable)
* Extend user profiles, e.g. avatar, jabber, etc.
* Polish geo location lookup
* Polish roles and user administration
* Mass mailer for admin
* Enrich map layer
...
Most of the authentication and roles authorization is taken from the tutorial by Matthew Solt, released under the MIT License
...
Copyright (c) 2009 Mikolas Bingemer